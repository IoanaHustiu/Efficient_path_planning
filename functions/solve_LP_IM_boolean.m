function [LPIM,ILPIM] = solve_LP_IM_boolean(num_intermediate,C,Post,At,bt,V,x,m0,flag_ILP)
LPIM.exitflag = 0;
ILPIM.exitflag = 0;
nplaces = size(C,1);
ntrans = size(C,2);
...
try
    ...
    fprintf(1,'LP with %d equality constraints and %d inequality constraints.\n',size(Aeq2,1),size(Aineq2,1));
    tic;
    ...
    if (exitflag2 ~= 1)
        ...
        if time_LP2>=1800
            fprintf('Solving the second problem (MILP) lasts more than 3600 seconds!\n');
            LPIM.exitflag = 3600;
        else
    ...
    if flag_ILP
        ...
        if (exitflag2 ~= 1)
            ...
            if time_LP2>=1800
                fprintf('Solving the second problem (ILP) lasts more than 3600 seconds!\n');
                ILPIM.exitflag = 3600;
            else
                ...
                x_ILP = ILPIM.sol(num_intermediate*(nplaces+ntrans)+1:num_intermediate*(nplaces+ntrans)+np);
