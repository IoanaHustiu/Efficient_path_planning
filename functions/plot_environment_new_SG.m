function env_h = plot_environment_new_SG(selectedStart, selectedFin, map2D, T)
...
% Draws free=white, obstacles=black; starts=red triangles, goals=gold diamonds.
    ...
    if nargin < 4, error('Usage: plot_environment_new_SG(S, G, map2D, T)'); end
    ...
    if size(S,1) ~= size(G,1)
        warning('Starts (%d) and goals (%d) differ. Using %d.', size(S,1), size(G,1), N);
    end
    S = S(1:N,:);  G = G(1:N,:);
    ...
    xlabel('x'); ylabel('y');
    title(sprintf('Starts (red) & Goals (gold) for %d robots', N));
    grid off; hold off;
end
