...
x = round(MILP1.sol(nplaces+ntrans+1:nplaces+ntrans+size(At,2)));
num_intermediate = s;
if num_intermediate > 1
    ...
    while flag_num
        ...
        if flag_ILP
            [LPIM,ILPIM] = solve_LP_IM_boolean(num_intermediate,Post-Pre,Post,At,V,x,m0,flag_ILP);
            ...
            LPIM = solve_LP_IM_boolean(num_intermediate,Post-Pre,Post,At,V,mf,m0,flag_ILP);
        end
...
if plot_animation
    ...
    for i = 1 : num_intermediate
        if (i == 1)
            ...
            current_marking = round(LPIM.sol((nplaces+ntrans+np)*(i-2)+1:(nplaces+ntrans+np)*(i-2)+nplaces));
        end
        % figure;
        ...
        if num_intermediate == 1
            ...
            current_sigma = round(LPIM.sol((nplaces+ntrans+np)*(i-1)+nplaces+1:(nplaces+ntrans+np)*(i-1)+nplaces+ntrans));
        end
    ...
    if flag_ILP
        ...
        for i = 1 : num_intermediate
            if (i == 1)
                ...
                current_marking = round(ILPIM.sol((nplaces+ntrans+np)*(i-2)+1:(nplaces+ntrans+np)*(i-2)+nplaces));
            end
            %figure;
            plot_environment(T.rem_cell(find(current_marking)),T.rem_cell(T.props),T.map2D,env_limit,T.Vert);
            ...
            if num_intermediate == 1
                ...
                current_sigma = round(ILPIM.sol((nplaces+ntrans+np)*(i-1)+nplaces+1:(nplaces+ntrans+np)*(i-1)+nplaces+ntrans));
            end
